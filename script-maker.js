...
async function runScriptAutomation(rawText, taskId, skipBgm) { // 🌟 THÊM BIẾN skipBgm VÀO ĐÂY
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        // AUTO BGM: Nếu đã đọc được khoảng 400 chữ (tương đương ~3-4 phút audio)
        if (wordCount >= 400) {
...
async function exportTimestampTxtFiles(timestampLog, runningTime, taskId) {
    ...
    '',
    '🆕 THAY ĐỔI QUAN TRỌNG SO VỚI V3.1:',
    '    • QUALITY TAGS mới: masterpiece, high score, great score, absurdres',
    '      (KHÔNG dùng "best quality, very aesthetic" như v3.1 nữa)',
    '    • CFG Scale giảm xuống 5 (v3.1 dùng 7)',
    '    • Score Tags + Quality Tags CỘNG HƯỞNG — dùng cả hai để tối đa chất lượng',
    '    • Temporal Tags mới: year 2024 / year 2025 — điều chỉnh art style',
    '    • NEGATIVE PROMPT đơn giản hơn — xem PHẦN 1',
    '    • SERIES TAG bắt buộc ngay sau CHARACTER TAG',
    '',
    SEP50,
    'THỨ TỰ TAG BẮT BUỘC (OFFICIAL — TRAIN THEO THỨ TỰ NÀY):',
    SEP50,
    '',
    '  [1] GENDER TAG      : 1boy / 1girl / 2boys / 1boy 1girl / no humans',
    '  [2] CHARACTER NAME  : tên nhân vật (nếu có) — ví dụ: xu fan',
    '  [3] SERIES/SOURCE   : tên bộ truyện ← BẮT BUỘC ngay sau [2]',
    '  [4] RATING TAG      : safe ← ĐẶT SỚM sau series tag (theo cấu trúc official)',
    '  [5] CONTENT TAGS    : tất cả mô tả chi tiết (tóc, mắt, trang phục,',
    '                         hành động, bối cảnh, ánh sáng, góc máy)',
    '  [6] TEMPORAL TAG    : year 2024 / year 2025 ← tùy chọn, điều chỉnh art style',
    '  [7] QUALITY TAGS    : masterpiece, high score, great score, absurdres ← CUỐI',
    '',
    ...
    '',
    SEP50,
    'SCORE TAGS vs QUALITY TAGS — CÁCH DÙNG KẾT HỢP:',
    SEP50,
    '',
    '  QUALITY TAGS (kiểm soát mức độ tinh xảo tổng thể):',
    '    masterpiece  → ảnh đạt chuẩn tác phẩm nghệ thuật',
    '    best quality → chất lượng đường nét và màu sắc cao nhất',
    '',
    '  SCORE TAGS (kiểm soát điểm thẩm mỹ — MẠNH HƠN quality tags ở model 4.0):',
    '    high score   → điểm thẩm mỹ cao, bố cục đẹp, màu sắc hài hòa',
    '    great score  → điểm nghệ thuật xuất sắc',
    '    good score   → chất lượng tốt (dùng khi muốn ảnh ít "over-processed")',
    '',
    '  ✅ COMBO TỐI ƯU: masterpiece, high score, great score, absurdres',
    '  ✅ COMBO NHẸ HƠN (ít artifact): masterpiece, good score, absurdres',
    '',
    SEP50,
    'TEMPORAL TAGS — ĐIỀU CHỈNH ART STYLE THEO NĂM:',
    SEP50,
    '',
    '  year 2005  → art style anime cũ, nét đơn giản, màu phẳng',
    '  year 2015  → style anime trung gian, khá phổ biến',
    '  year 2024  → style anime hiện đại, màu rực, detail cao',
    '  year 2025  → style mới nhất, dynamic, cinematic hơn',
    '',
    '  ✅ KHUYẾN NGHỊ cho truyện xianxia: year 2024 hoặc year 2025',
    '  📌 Đặt temporal tag TRƯỚC quality tags ở cuối prompt',
    '',
    SEP50,
    'RECOMMENDED SETTINGS (COLAB PIPELINE):',
    SEP50,
    '  • Pipeline    : lpw_stable_diffusion_xl  ← QUAN TRỌNG (xử lý prompt dài tốt hơn)',
    '  • CFG Scale   : 5  (range 4-7, khuyến nghị chính thức 4.0)',
    '  • Steps       : 28',
    '  • Sampler     : Euler Ancestral (Euler a)',
    '  • Resolution  : 1344 x 768  (16:9 landscape — cho video YouTube)',
    '  • Alt 16:9    : 1216 x 832  (action/landscape)',
    '  • Portrait    : 832 x 1216  (character close-up)',
    '  • Square      : 1024 x 1024 (scene/cultivation)',
    '',
    '  ⚠️  CHỐNG TRÀN VRAM — THÊM DÒNG NÀY VÀO COLAB:',
    '      pipe.enable_model_cpu_offload()  ← giảm VRAM từ 11GB xuống 5.6GB',
    '      KHÔNG dùng .to("cuda") khi đã dùng cpu_offload',
    '',
    SEP50,
    'NEGATIVE PROMPT CHUẨN OFFICIAL (v4.0):',
    SEP50,
    ...
    '',
    'MỖI SCENE GROUP = 1 ẢNH, gom nhiều dòng lại để đủ ngữ cảnh (~15-30 giây)',
    '',
    'QUY TẮC CỨNG:',
    ...
    '',
    SEP80,
    'PHẦN 2 — TEMPLATE PROMPT DANBOORU (ANIMAGINE XL 4.0)',
    SEP80,
    '',
    'TEMPLATE CHUẨN (theo official structure):',
    '  1boy/1girl, [character name], [series], [rating],',
    ...
    SEP50,
    '',
    '  [IMAGE PROMPT: 1boy, xu fan, xianxia, safe,',
    ...
    '  dappled sunlight through bamboo leaves, soft sidelighting,',
    '  warm golden rim light on hair, cool green shadow,',
    ...
    SEP50,
    '',
    '  [IMAGE PROMPT: 1boy, xianxia, chinese fantasy, safe,',
    ...
    '  longsword, qi energy slash, dynamic combat stance, motion blur on sword,',
    '  stone ruins battlefield, broken pillars, debris mid-air,',
    '  backlighting, golden afternoon light, harsh rim light,',
    '  volumetric light rays, dust particles in light, bloom effect,',
    '  warm amber vs cool blue contrast,',
    '  extreme low angle, diagonal composition, left-third framing,',
    '  year 2025,',
    '  masterpiece, high score, great score, absurdres]',
    '  [NEGATIVE: lowres, bad anatomy, bad hands, text, error, missing finger,',
    '  extra digits, fewer digits, cropped, worst quality, low quality,',
    ...
    '',
    SEP50,
    'VÍ DỤ 3 — TIÊN CẢNH TU LUYỆN (cultivation/breakthrough scene):',
    SEP50,
    '',
    '  [IMAGE PROMPT: 1girl, xianxia, chinese cultivation, safe,',
    ...
    '  lotus mudra, meditation, floating, lotus position,',
    '  sea of clouds, floating islands, jade pavilion distant,',
    '  light rays, golden dawn sunbeams, bloom,',
    '  divine backlighting, rim light halo, soft blue ambient light,',
    '  spiritual energy ribbons, glowing spirit particles, subsurface scattering,',
    '  wide shot, slightly below eye level, deep depth of field,',
    '  year 2025,',
    '  masterpiece, high score, great score, absurdres]',
    '  [NEGATIVE: lowres, bad anatomy, bad hands, text, error, missing finger,',
    '  extra digits, fewer digits, cropped, worst quality, low quality,',
    ...
    '',
    SEP50,
    'VÍ DỤ 4 — CẢNH PHONG CẢNH (scenery — no humans):',
    SEP50,
    '',
    '  [IMAGE PROMPT: scenery, no humans, safe,',
    '  mountain range, ancient stone path, ruined stone steps,',
    '  cherry blossom trees, falling petals, storm clouds gathering,',
    '  fog rolling through valley, distant pagoda silhouette,',
    '  light rays, golden hour, bloom, dappled sunlight,',
    '  dramatic sky, dark atmosphere,',
    '  wide shot, cinematic composition, deep depth of field,',
    '  year 2025,',
    '  masterpiece, high score, great score, absurdres]',
    '  [NEGATIVE: lowres, bad anatomy, bad hands, text, error, missing finger,',
    '  extra digits, fewer digits, cropped, worst quality, low quality,',
    ...
    '',
    SEP50,
    'VÍ DỤ 5 — CẢNH NỘI TÂM / CẢM XÚC (emotional/aftermath scene):',
    SEP50,
    '',
    '  [IMAGE PROMPT: 1boy, xianxia, safe,',
    '  black hair, disheveled, dark brown eyes, downcast eyes,',
    '  white hanfu, torn and bloodstained, bandaged hand,',
    '  kneeling on ground, exhausted, head bowed, fist on ground,',
    '  destroyed battlefield, ash falling, embers floating,',
    '  sunset, underlighting from embers, sidelighting warm,',
    '  silhouette against red sky, bloom on horizon,',
    '  close-up, warm backlight, soft golden rim light,',
    '  year 2025,',
    '  masterpiece, high score, great score, absurdres]',
    '  [NEGATIVE: lowres, bad anatomy, bad hands, text, error, missing finger,',
    '  extra digits, fewer digits, cropped, worst quality, low quality,',
    ...
    '            wide eyes / half-closed eyes / glowing eyes',
    '',
    SEP50,
    ...
    '  torn clothing / bloodstained clothing / disheveled appearance',
    '  jade pendant / jade ring / spiritual treasure / longsword / flying sword',
    '',
    SEP50,
    'ACTION/POSE TAGS:',
    SEP50,
    '  standing / sitting / walking / running / flying / leaping / falling',
    '  sword slash / qi release / energy blast / casting spell',
    '  meditation / lotus position / floating / arms crossed / arms behind back',
    '  dynamic pose / battle stance / kneeling / exhausted / reaching out',
    '  motion blur / speed lines / impact frame / dramatic pause',
    '',
    SEP50,
    'SETTING — XIANXIA:',
    SEP50,
    ...
    '  destroyed battlefield / burning village / snow field / cliff edge',
    '  stormy sky / lightning / ash falling / embers floating',
    '',
    SEP50,
    'LIGHTING TAGS (DANBOORU OFFICIAL):',
    SEP50,
    '  Hướng ánh sáng:',
    '    backlighting     → sáng từ sau lưng, tạo viền sáng (epic/dramatic)',
    '    sidelighting     → sáng từ một bên, tạo bóng đổ rõ nét',
    '    underlighting    → sáng từ phía dưới, tạo cảm giác huyền bí/ominous',
    '    overlighting     → sáng từ trên cao, tạo bóng mặt',
    '',
    '  Hiệu ứng ánh sáng:',
    '    bloom            → vầng sáng lan tỏa quanh nguồn sáng',
    '    light rays       → tia sáng xuyên qua (volumetric)',
    '    spotlight        → ánh đèn sân khấu chiếu thẳng',
    '    dappled sunlight → ánh nắng lọc qua lá cây',
    '    subsurface scattering → ánh sáng xuyên qua vật thể mỏng (da, vải)',
    '    caustics         → ánh sáng phản chiếu qua nước',
    '',
    '  Thời điểm trong ngày:',
    '    dawn / sunrise   → bình minh, màu hồng cam nhẹ',
    '    golden hour      → hoàng hôn/bình minh, ánh vàng ấm',
    '    sunset / dusk    → hoàng hôn, đỏ cam mạnh',
    '    moonlight        → ánh trăng, xanh lạnh',
    '    night            → ban đêm',
    '',
    '  Không khí:',
    '    dark atmosphere  → không khí tối tăm, nặng nề',
    '    ethereal glow    → ánh sáng huyền ảo',
    '    fire glow        → ánh lửa đỏ cam',
    '    energy aura      → hào quang năng lượng',
    '    spiritual particles → hạt năng lượng linh khí',
    '',
    SEP50,
    'COMPOSITION TAGS:',
    SEP50,
    '  wide shot / medium shot / close-up / extreme close-up / cowboy shot',
    '  upper body / full body / portrait / bust',
    '  low angle / high angle / eye level / bird\'s eye view / worm\'s eye view',
    '  depth of field / shallow focus / bokeh / blurry background',
    '  dutch angle / diagonal composition / dynamic angle / rule of thirds',
    '  silhouette / dramatic framing / cinematic letterbox',
    '',
    SEP50,
    'TEMPORAL TAGS (ĐỂ ĐIỀU CHỈNH STYLE):',
    SEP50,
    '  year 2015  → style anime truyền thống, đơn giản',
    '  year 2020  → style cân bằng, ổn định',
    '  year 2024  → style hiện đại, màu sắc rực rỡ',
    '  year 2025  → style mới nhất, cinematic, dynamic ← KHUYẾN NGHỊ',
    '',
    SEP80,
    ...
    '      → special features (scar, beauty mark, weapon, accessory)',
    '  B3: COPY y chang tag set đó vào mọi scene nhân vật đó xuất hiện',
    '  B4: CHỈ ĐỔI: action tags, expression tags, setting tags, lighting tags',
    '  B5: KHÔNG BAO GIỜ đổi: hair color, eye color, clothing color, features',
    '',
    '  VÍ DỤ NHẤT QUÁN:',
    '  Scene 1: 1boy, xu fan, xianxia, safe, black hair, dark brown eyes,',
    '           white hanfu, jade ring, standing, bamboo courtyard,',
    '           dappled sunlight, year 2025,',
    '           masterpiece, high score, great score, absurdres',
    '  Scene 2: 1boy, xu fan, xianxia, safe, black hair, dark brown eyes,',
    '           white hanfu, jade ring, battle stance, ruins, backlighting,',
    '           year 2025,',
    '           masterpiece, high score, great score, absurdres',
    '           ↑ hair/eyes/clothing GIỐNG HỆT — chỉ đổi pose, setting, lighting',
    '',
    SEP80,
